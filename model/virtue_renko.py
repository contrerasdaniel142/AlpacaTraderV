...
class virtueRenko:
    ...
    def _Find_first_ideal_brick(self):
        ...
        for index, row in self.dfRates.iterrows():
            if (row['high'] - row['low']) >= self.brick_size * 2:
                primer_valor = row
                break
    ...
    def price_adjustment_to_brick_size(self, price):
        if price < self.brick_size:
            price = int(price) + self.brick_size
        elif price > self.brick_size:
    ...
    def create_renko(self):
        index = 0
        fcount = 0
        # price_line simulara la linea de precio desde la barra inicial hasta que llegue a la ultima barra
        price_line = 0
        while index < len(self.dfRates):
            ...
            if fcount == 0:
                ...
                next_bar = self.dfRates.iloc[index + 1] if index + 1 < len(self.dfRates) else self.dfRates.iloc[index]

                # Se establece la linea de precio con el precio con el que abre la barra
                price_line = current_bar["open"]
                # Se revisa si la primera barra sube o baja con su precio de entrada y cierre
                if current_bar["open"] < current_bar["close"]:
                    # Sube, por lo tanto se ajustara con ello el precio de entrada con el tamaño de los ladrillos establecido
                    if price_line 
                ...
                next_bar = self.dfRates.iloc[index + 1] if index + 1 < len(self.dfRates) else self.dfRates.iloc[index]
                if not last_brick:
                    last_brick["open"] = self.price_adjustment_to_brick_size(current_bar["open"])
                    last_brick["close"] = self.price_adjustment_to_brick_size(current_bar["close"])
                    last_brick["high"] = current_bar["high"]
                    last_brick["low"] = current_bar["low"]                        
                delta = abs(current_bar["high"] - current_bar["low"])
                fcount = int(delta / self.brick_size)
                index = index + 1
            else:
                special_brick_up = False
                special_brick_down = False
                current_bar["open"] = self.price_adjustment_to_brick_size(current_bar["open"])
                current_bar["close"] = self.price_adjustment_to_brick_size(current_bar["close"])
                if len(self.bricks) == 0:
                    if last_brick["open"] + self.brick_size <= current_bar["high"]:
                        type="up"
                    elif last_brick["open"] - self.brick_size >= current_bar["low"]:
                        type = "down"
                    first_open_price = last_brick["open"]
                ...
                if special_brick_up:
                    ...
                    self._add_brick(type, current_bar["time"], first_open_price, next_bar["low"])
                elif type == "down":
                    self._add_brick(type, current_bar["time"], first_open_price, next_bar["high"])  
                fcount -= 1
    ...
    def _add_brick(self, type: str, time, first_open_price: float= None, wick: float = None):
        if len(self.bricks) == 0:
            if type == "up":
                ...
                open_price = first_open_price
                close_price = first_open_price - self.brick_size
                high_price = first_open_price
                low_price = first_open_price - self.brick_size if wick == None else wick
        else:
